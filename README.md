...
- Orquestrador B9 Pulse com especialistas de diagnóstico, conciliação e resolução.
- Fallback determinístico sem chave de API para demos e desenvolvimento.
- MCP local com diagnóstico de notas, busca e controle do SLA de cinco minutos.
- Plug-in de repositório com skills de resolução fiscal e integração Lovable.
...
flowchart LR
    ...
    API --> ORQ[Orquestrador B9 Pulse]
    ORQ --> D[Diagnóstico]
    ORQ --> R[Conciliação]
    ORQ --> A[Resolução]
    D & R & A --> DATA[MCP / APIs B9]
    ORQ --> TRACE[Traces e SLA]
